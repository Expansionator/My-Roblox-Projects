...
function DataCacher:LoadProfile(Player: Player): Profile?
	...
	do
		...
		--[[
			...
			• It follows the CAP theorem, focusing on Consistency and Partition Tolerance
			• It also follows a hybrid of FIFO and LIFO principles, giving the latest server in the queue a chance to load the profile while other servers wait their turn

			• A queue is used to track servers attempting to load the same profile

			• There is a lock that the current server owning the profile is expected to refresh
			• This is essentially a lease-based lock, where a server does not own the profile indefinitely but retains it as long as it can provide a heartbeat to update the lock

			• The system follows the principle of allowing the current server that owns the profile to retain ownership
			...
			• The system enters a while loop, also referred to as "attempts", for each iteration
			• The while loop continues as long as these conditions are met:
			...
			• The "lock" refers to a Unix timestamp indicating when a server last accessed the profile and successfully modified it, such as saving
			• The lock primarily serves as a constant indicator that the server holding the profile is still active
			...
			• For every first attempt in a cycle (odd-numbered):
				• Keeps a record (JobId) of the latest requester in the queue
				• This "snapshot" is used to verify stability in the next attempt

			• For every second attempt in a cycle (even-numbered):
				• Ensures that a server doesn't get stuck behind an inactive or "ghost" server that joined the queue but crashed
				• Detect if the queue is stuck and clear the path for older requests
				...
				• If, after a full cycle, the latest server isn’t the current one, it’s assumed unresponsive or slow and is removed from the queue
				• This allows the queue to rotate, eventually moving the current server to the latest position so it can finally claim the session

			--------------------------------------------------------------------

			[ Queue ]

			• A table stored in the profile's metadata tracks the servers attempting to load the same profile
			• Since new values can be added to the table, and with the atomic behavior of UpdateAsync(), this prevents overwriting and ensures the order of servers remains sequential and deterministic
			...
			• The latest server in the queue is allowed to attempt reclaiming the profile during a cycle before being removed by other servers, if any exist
				• If no other servers are competing, the latest server will continue retrying until the profile is reclaimed or the maximum allowed cycles are reached

			• The other servers in the queue will eventually handle the latest server, while ensuring that only one server is allowed to perform the removal
			...
			• There are some special characteristics by using a queue:
				...
				• For the latest server in the queue:
					• If it is dead (e.g., crashed), it is automatically managed by the other servers in the queue. If no servers remain, a new server entering the queue will overwrite the old server
					• Since it is dead, it cannot remove the latest server that just entered the queue, allowing the new server to continue retrying without disrupting the entire mechanism
				...
				• For other servers in the queue:
					• Only a single server is allowed to remove the latest server from the queue
						...
						• This helps prevent timing conflicts, as every server must confirm it is still observing the same latest server
					...
					• Using servers A (oldest), B, and C (latest) as examples in the queue:
						...
						• If server B is dead, server A or C will take over handling server B
			...
			• Now two servers have access to the same profile, creating a critical risk of data corruption or loss
				...
				• This nearly guarantees that only two servers can simultaneously own the same profile
