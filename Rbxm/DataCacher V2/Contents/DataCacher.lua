...
• Retrieves the player's profile if it is currently loaded in memory. The optional Raw parameter returns the full profile, including session metadata.
...
local SESSION_KEY = "Session" --// For "__Session"

local LAST_CHARACTER_SUFFIX = "DC" --// For IndexingName, the characters appended to the key if there's a duplicate

local SESSION_TEMPLATE = { --// System variables associated with the player's profile
...
export type Profile = {}
export type RawProfile = {__Data: Profile, __Session: SessionData}
...
local function IsV1Format(data) --// DataCacher V1 support	
	local oldStructure = {
...
local function GetSessionData()
	local session = Clone(SESSION_TEMPLATE)
	return session
end
...
function DataCacher.CreateDatastore(Name: string, Settings: Config.Datastore?)
	...
	do
		...
		self.WaitForPlayerState = function(profileKey, iterate)
			...
			--// Keep waiting until the state is available
			if not iterate then
				repeat task.wait()
			...
			while attempt < totalAttempts or IsServerClosing do
				...
				success, newData = pcall(function()
					...
					local function Transform(data)
						local sessionTemplate = GetSessionData()
						...
						Reconcile(data[sessionKey], sessionTemplate)

						do

							local session = data[sessionKey]
							local sessionData = session.Session

							local currentSession = profile and profile[sessionKey]

							do

								--// Manages conditions associated with session locking

								local sessionReady = IsSessionReadyForUse(sessionData.JobId, sessionData.PlaceId, params.Action ~= "Loading")
								local isReclaimsInList = params.Action == "Loading" and #sessionData.ReclaimRequests > 0
							...
							if profile and currentSession.Version ~= session.Version then
		...
		while isLatestRequest and not IsServerClosing and table.find(pendingListTable, profileKey) and totalAttempts < maxProfileReclaimAttempts do
			...
			profile, isAttemptingToReclaim = self.Request(profileKey, {
				...
				ManageSession = function(latestData)
					...
					local session = latestData[sessionKey]
					local sessionData = session.Session

					local reclaimRequests = sessionData.ReclaimRequests
					...
					local lastUpdated = session.LastUpdated
					local currentJobId = sessionData.JobId
					...
					if IsServerTheLatest(jobId) and (IsSessionReadyForUse(currentJobId, sessionData.PlaceId) or (lockExpiryDiff and lockExpiryDiff >= expiryTime)) then
				...
				Transform = function(latestData)
					...
					sessionData.PlaceId = game.PlaceId

					--// To start fresh without any servers wanting to reclaim the profile
					table.clear(sessionData.ReclaimRequests)
				end
...
If Raw is true, this method returns the raw profile format, including __Data and __Session.
]]
function DataCacher:GetProfile(Player: Player, Raw: boolean?): Profile? | RawProfile?
	...
	--// Returns the profile with either only the actual data or both actual and session data
...
function DataCacher:SaveProfile(Player: Player, AutoSave: boolean?, ValueCallback: ValueCallback?): boolean?
	...
	local newProfile, ignoreSessionRequest = self.Request(profileKey, {
		...
		RevertSession = function(latestData)
			...
			local currentSession = profile[sessionKey]
		...
		Transform = function(latestData)
			...
			if not AutoSave then
				...
				sessionData.JobId = nil
				sessionData.PlaceId = nil
			end
		end
...
function DataCacher:WipeProfile(Player: Player)
	...
	--// Replaces the profile data with the template while retaining its session data
...
function DataCacher:GetProfileState(ProfileKey: string): ProfileState?
	...
	if not state and (DatastoreEnabled or MockDatastoreEnabled) then
		...
		do
			...
			local sessionTable = profile[sessionKey]
			if not sessionTable or typeof(sessionTable) ~= "table" then
				return
			end

			local unknownState = sessionTable.State
